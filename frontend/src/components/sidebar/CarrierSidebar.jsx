...
export default function CarrierSidebar() {
  ...
  return (
    <div className="w-[260px] min-w-[260px] h-screen bg-dat-dark flex flex-col justify-between border-r border-dat-border font-outfit overflow-hidden">
      <div className="pt-5">
          ...
          {navItems.map(({ icon: Icon, label, path }) => {
            ...
            return (
              <li key={label}>
                <Link
                  to={path}
                  className={`flex items-center gap-3.5 px-6 py-2 cursor-pointer transition-all duration-150 border-l-[3px] text-sm font-normal tracking-[0.3px] no-underline
                    ${
                      active
      ...
      <div className="py-3 border-t border-dat-border">
        {bottomItems.map(({ icon: Icon, label }) => (
          <div
            key={label}
            className="flex items-center gap-3.5 px-6 py-2 cursor-pointer transition-all duration-150 text-[#374151] border-l-[3px] border-l-transparent hover:bg-white/[0.04] hover:text-[#9ca3af] text-[13px]"
          >
